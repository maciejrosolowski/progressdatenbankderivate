...
#' the column "patstuid". Columns "kate_<name of the time point>" contain the data
#' on KATECHO in the wide format. Columns "kat.atall", "kat.d0", "kat.ab.d1" are
...
getData4kate <- function(FRM_KAT) {
  ...
  EVENT <- PATSTUID <- KATECHO <- event <- patstuid <- value <-
    kat.atall <- kat.d0 <- kat.ab.d1 <- event <- NULL
  ...
  toadd_kate <- copy(kate)
  toadd_kate[, event := paste("kate", event2zeitpunkt(
    event, returnformat = "zp_fabianref"), sep = "_")]
  toadd_kate <- dcast(toadd_kate, patstuid ~ event)

  # merge with the additional columns
  toadd_kate <- merge(toadd_kate, kat.atall, by = "patstuid",
                      all = TRUE, sort = FALSE)
  toadd_kate  = merge(toadd_kate, kat.d0, by = "patstuid", all  =T ,sort = F)
