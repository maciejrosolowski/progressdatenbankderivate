...
getData4verwirrt <- function(FRM_BEF) {
  ...
  toadd_verwirrt = FRM_BEF[,.(patstuid = PATSTUID, event = EVENT,value= as.numeric(VERWIRRTH))]
  ...
  setnames(toadd_verwirrt,  as.character(found_events_verwirrt), paste0("verwirrt_",event2zeitpunkt(found_events_verwirrt, returnformat = "zp_fabianref")))
